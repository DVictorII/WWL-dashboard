...
def create_chart(paddock, piezo, days, initial_pressure, dates):
    ...
    plt.fill_between(t, s, min(pressure), color=["#477C9A"], alpha=0.1)
...
def create_pdf(
    ...
    if len(lecturesDates) != 0:
        ...
        pdf.cell(
            50,
            10,
            f"Average PWP (From {lecturesDates[0]} to {lecturesDates[len(lecturesDates)-1]}): ",
        )
    ...
    if len(inoperativeDates) != 0:
        ...
        pdf.cell(
            50,
            10,
            f'Inoperative dates (From {inoperativeDates[0]["currentDate"]} to today): ',
        )
        ...
        for obj in inoperativeDates:
            pdf.cell(
                20,
                10,
                f'{obj["inoperativeDays"]} days: from {obj["currentDate"]} to {obj["nextDate"]}',
            )
